...
class MyInfoLogic extends GetxController {
    ...
    IMWidget.openPhotoSheet(onData: (path, url) async {
      if (url != null) {
        LoadingView.singleton.wrap(
          asyncFunction: () => Apis.updateUserInfo(userID: OpenIM.iMManager.uid, faceURL: url)
  ...
  void _updateGender(int gender) {
    LoadingView.singleton.wrap(
      asyncFunction: () => Apis.updateUserInfo(userID: OpenIM.iMManager.uid, gender: gender)
          .then((value) => imLogic.userInfo.update((val) {
                val?.gender = gender;
              })),
    );
  ...
  void _updateBirthday(int birthday) {
    LoadingView.singleton.wrap(
      asyncFunction: () => Apis.updateUserInfo(
              ...
              birthStr: DateUtil.formatDateMs(birthday * 1000, format: 'yyyy-MM-dd'))
          .then((value) => imLogic.userInfo.update((val) {
                val?.birth = birthday;
                val?.birthTime = DateUtil.formatDateMs(birthday * 1000, format: 'yyyy-MM-dd');
              })),
    );
  }
