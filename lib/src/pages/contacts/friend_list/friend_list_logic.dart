...
class MyFriendListLogic extends GetxController {
  ...
  var uidList = <String>[];
  String? _comment;
  late StreamSubscription friendDelSub;
  ...
  @override
  void onInit() {
    friendDelSub = imLoic.friendDelSubject.listen((user) {
      print('delete user:   ${json.encode(user)}');
      friendList.removeWhere((e) => e.userID == user.userID);
    });
    friendAddSub = imLoic.friendAddSubject.listen((user) {
      print('add user:   ${json.encode(user)}');
      // getFriendList();
      _addUser(user.toJson());
    });
    friendInfoChangedSub = imLoic.friendInfoChangedSubject.listen((user) {
