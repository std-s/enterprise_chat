...
class GroupMemberListPage extends StatelessWidget {
                    ...
                    itemBuilder: (context, data, index) {
                      ...
                      return InkWell(
                        ...
                        child: buildAzListItemView(
                          isMultiModel: logic.isEveryoneTag(data.nickname!)
                              ? false
                              : logic.isMultiModel(),
                          name: data.nickname!,
                          url: data.faceURL,
                          ...
                          enabled: !disabled,
                          isUserGroup: logic.isEveryoneTag(data.nickname!),
                        ),
                      );
                    },
                  ),
  ...
  Widget _buildItemView(int index) {
    final data = logic.memberList.elementAt(index);
    final disabled = logic.defaultCheckedUidList.contains(data.userID);
    final isEveryoneTag = logic.isEveryoneTag(data.nickname!);
    final isMultiModel = isEveryoneTag ? false : logic.isMultiModel();
    return disabled
        ? buildAzListItemView(
            ...
            checked: true,
            enabled: false,
            customAvatarView: isEveryoneTag ? SizedBox() : null,
            // isUserGroup: isEveryoneTag,
          )
        : Obx(() => InkWell(
              onTap: () => logic.selectedMember(index),
              child: buildAzListItemView(
                isMultiModel: isMultiModel,
                name: data.nickname!,
                url: data.faceURL,
                checked: logic.currentCheckedList.contains(data),
                enabled: true,
                customAvatarView: isEveryoneTag ? SizedBox() : null,
                // isUserGroup: isEveryoneTag,
              ),
